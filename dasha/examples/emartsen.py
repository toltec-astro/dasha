...
class AnExamplePage(ComponentTemplate):
    ...
    def setup_layout(self, app):
        ...
        # controling widgets and the other for view result
        ...
        def update_view(n_calls, select_value, input_value, checklist_value):
            ...
            self.logger.debug(
                    ...
                    f"input_value={input_value} checklist_value={checklist_value}")

            # we can compose a layout to return as the child of view
            # using pure dash.
            # No component template is allowed inside callback.

            # As an example, we do a simple calcuation using the inputs
            # note that it is often the case one need to check the value against None, which is
            # the default
